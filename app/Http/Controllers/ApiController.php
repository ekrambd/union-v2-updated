...
{   
    ...
    {   
        try {
            ...
            // Try Rider
            $rider = Rider::where($fieldType, $login)->first();
            ...
            // Try Lawyer
            $lawyer = Lawyer::where($fieldType, $login)->first();
            if ($lawyer && Hash::check($password, $lawyer->password)) {
                $lawyer->load('lawyerdoc','lawyeravailability','lawyerfee');
                if ($lawyer->status == 'Inactive') {
                    return response()->json(['status'=>false, 'role'=>"", 'message'=>'Your account not active yet', 'token'=>"", 'data'=>new \stdClass()],403);
                }
    ...
    {
        try{
            $data = Doctorappointment::with('doctor.doctordoc','patientinfo','doctor.doctorfee','doctorconversation')->where('user_id',user()->id)->where('status','booked')->latest()->get();
            return response()->json(['status'=>count($data)>0, 'data'=>$data]); 
        }catch(Exception $e){
            return response()->json(['status'=>false, 'code'=>$e->getCode(), 'message'=>$e->getMessage()],500);
        }
    ...
    public function myLawyerAppointments(Request $request)
    {
        ...
        {   
            $date = date('Y-m-d'); 
            $data = Lawyerappointment::with('lawyerconversation','lawyer.lawyerdoc','lawyer.lawyerfee','lawyer.lawyeravailability','userinfo')->where('user_id',user()->id)->where('appointment_date','>=',$date)->orderBy('appointment_date','ASC')->paginate(15);  
            return response()->json($data);
        }catch(Exception $e){
            return response()->json(['status'=>false, 'code'=>$e->getCode(), 'message'=>$e->getMessage()],500);
        }
