...
{   
    ...
    public function myLawyerAppointments(Request $request)
    {
        ...
        {   
            $date = date('Y-m-d'); 
            $data = Lawyerappointment::with('lawyer.lawyerdoc','lawyer.lawyerfee','lawyer.lawyeravailability','userinfo','lawyerconversation')->where('user_id',user()->id)->where('appointment_date','>=',$date)->orderBy('appointment_date','ASC')->paginate(15);  
            return response()->json($data);
        }catch(Exception $e){
            return response()->json(['status'=>false, 'code'=>$e->getCode(), 'message'=>$e->getMessage()],500);
        }
